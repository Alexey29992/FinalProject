...
<head>
    ...
    <title><fmt:message key="nav_bar.client.request_history"/></title>
        ...
        <table>
            ...
            <c:forEach var="row" items="${requestScope.requests}">
                <tr>
                    ...
                    <td>
                        <c:if test="${row.status eq 'WAIT_FOR_PAYMENT'}">
                            <form method="post" action="${pageContext.request.requestURI}">
                                <input type="hidden" name="command" value="make-payment"/>
                                <button name="request-id" value="${row.id}" class="action-button">
                                    <fmt:message key="button.pay"/>
                                </button>
                            </form>
                        </c:if>
                        <c:if test="${row.status eq 'DONE' && empty row.userReview}">
